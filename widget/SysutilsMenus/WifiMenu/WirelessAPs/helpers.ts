...
import { DEVICE_STATES } from "../network";
...
export const staging = Variable<AstalNetwork.AccessPoint | undefined>(
	undefined,
);
...
const wifiEnabled = (): void => {
	...
	wifiEnabledBinding = Variable.derive(
...
const dedupeWAPs = (): AstalNetwork.AccessPoint[] => {
	...
	WAPs.forEach((item: AstalNetwork.AccessPoint) => {
		if (
			item.ssid !== null &&
			!Object.prototype.hasOwnProperty.call(dedupMap, item.ssid)
		) {
 ...
 */
export const isApEnabled = (
	state: AstalNetwork.DeviceState | undefined,
): boolean => {
	if (!state) {
		return false;
	}
...
export const connectToAP = (
	...
	if (
		accessPoint.bssid === connecting.get() ||
...
export const disconnectFromAP = (
	...
	execAsync("nmcli connection show --active").then((res: string) => {
		...
		if (connectionId === undefined) {
			console.error(
				`Error while disconnecting "${accessPoint.ssid}": Connection ID not found`,
			);
			return;
		}
		...
		execAsync(`nmcli connection down ${connectionId} "${accessPoint.ssid}"`)
			...
			.catch((err: unknown) => {
				connecting.set("");
				console.error(
					`Error while disconnecting "${accessPoint.ssid}": ${err}`,
				);
			});
	});
};
...
export const forgetAP = (
	...
	execAsync("nmcli connection show --active").then((res: string) => {
		...
		if (connectionId === undefined) {
			console.error(
				`Error while forgetting "${accessPoint.ssid}": Connection ID not found`,
			);
			return;
		}
